        ...
        elseif($temp2>=3 && $temp2<=8){
            for($col=1;$col<=14;$col++)
            {
                ...
                elseif($col<=13)
                {
                    ...
                    {
                        $str="<td><input type=\"number\" readonly=\"readonly\" name=t".$temp1."r".intval($temp2-2)."c".intval($col-1)." min=\"0\" max=\"999\" ></td>";
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    $table.="<td><output name=t".$temp1."r".intval($temp2-2)."c".intval($col-1)." size=\"2\">".intval($offbandcont[$temp2-3])."<td>".floatval(($offbandcont[$temp2-3]/$projectrevenue)*100)."%</output></td>";//calculation
                }
            }
        }
        elseif($temp2==9) 
...
//                    $table.="<td><output name=\"\"></output></td>"; 
                ...
                else
                {
                    ...
                    $table.="<td style=\"color:#FF6262;\"><output><b>".array_sum($offbandcont)."<td>".floatval((array_sum($offbandcont)/$projectrevenue)*100)."%</b></output></td>"; //calculation
                }
            }
        }
        elseif($temp2==10)
        ...
        elseif($temp2>=11 && $temp2<=16)
        {
            for($col=1;$col<=14;$col++)
            {
                ...
                else
                {
                    ...
                    $table.="<td><output name=t".$temp1."r".intval($temp2-10)."c".intval($col-1)." size=\"2\">".intval($onbandcont[$temp2-11])."<td>".floatval(($onbandcont[$temp2-11]/$projectrevenue)*100)."%</output></td>"; //calculation
                }
            }
        }
        else    //    elseif($temp2==17) 
        {
            for($col=1;$col<=14;$col++)
            {
                ...
                {
                    $table.="<td style=\"color:#FF6262;\"><output><b>".array_sum($onbandcont)."<td>".floatval((array_sum($onbandcont)/$projectrevenue)*100)."%</b></output></td>"; //calculation
                }
            }
        }
        $table.="</tr>";    
    ...
    $profit=floatval((($projectrevenue-$totalrate)/$projectrevenue)*100);//formula and calculation of profit
    ...
    for($temp1=1;$temp1<=$noofyears;$temp1++)//temp1---year
    {
        ...
        for($k=1;$k<=12;$k++)//rows
        {
              ...
              if($k<=6)
                ...
                $updatevaluesquery .= "UPDATE `projectresources` SET `M1`=$var[1],`M2`=$var[2],`M3`=$var[3],`M4`=$var[4],`M5`=$var[5],`M6`=$var[6],`M7`=$var[7],`M8`=$var[8],`M9`=$var[9],`M10`=$var[10],`M11`=$var[11],`M12`=$var[12],`HeadCount`=".array_sum($var).", `Rate`=".intval(array_sum($var)*$sal).",  `Contribution`=".floatval(array_sum($var)*$sal*100/$projectrevenue)." WHERE `ProjectID`='$projectid' AND  `Year`=".intval($temp1)." AND `LOCATION`='$loc' AND `Band`='".$band."';";
        }
