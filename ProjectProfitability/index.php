...
<!--        Template End-->
		...
		<?php
			...
			if(isset($_POST["login"]))
			{
				...
				$res=mysql_query("select * from loginaccess where username='$user' and password='$pass';");
				if(mysql_num_rows($res))
				{
                    //Query to retrieve Role and FName data from database
                    $ret = mysql_query("select Role,FName from loginaccess where username = '$user';");
                    while($row = mysql_fetch_assoc($ret)) {  //Fetching data from the database
                        $role = $row["Role"];
                        $en = $row["FName"];
                    }
                    switch($role)
                    {
                        case 'PM': if($pass == "Techm@".$user."$"){ ?><script>alert("Welcome, You need to change your password first before you login");location.href = "newpassword.php";</script><?php } //Redirect to password changing page
                                    if($pass != "Techm@".$user."$")
                                    {
                                        ?><script>alert("Log in Successfull"); location.href = "dashboardpm.php";</script><?php  
                                    }//Redirect to PM dashboard
                                    
                        case 'PGM': if($pass == "Techm@".$user."$"){ ?><script>alert("Welcome, You need to change your password first before you login");location.href = "newpassword.php";</script><?php } //Redirect to password changing page
                                    if($pass != "Techm@".$user."$")
                                    {
                                        ?><script>alert("Log in Successfull"); location.href = "dashboardpgm.php";</script><?php  
                                    }  //Redirect to PGM dashboard
                        case 'IBU': if($pass == "Techm@".$user."$"){ ?><script>alert("Welcome, You need to change your password first before you login");location.href = "newpassword.php";</script><?php } //Redirect to password changing page
                                    if($pass != "Techm@".$user."$")
                                    {
                                        ?><script>alert("Log in Successfull"); location.href = "dashboardibu.php";</script><?php  
                                    }  //Redirect to IBU dashboard
                        case 'IBG': if($pass == "Techm@".$user."$"){ ?><script>alert("Welcome, You need to change your password first before you login");location.href = "newpassword.php";</script><?php } //Redirect to password changing page
                                    if($pass != "Techm@".$user."$")
                                    {
                                        ?><script>alert("Log in Successfull"); location.href = "dashboardibg.php";</script><?php  
                                    }  //Redirect to IBG dashboard
                        case 'Admin': if($pass == "Techm@".$user."$"){ ?><script>alert("Welcome, You need to change your password first before you login");location.href = "newpassword.php";</script><?php } //Redirect to password changing page
                                    if($pass != "Techm@".$user."$")
                                    {
